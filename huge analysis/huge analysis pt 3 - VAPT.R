...
variability_overall_vapt <- daily_eligible %>%
  group_by(user_id) %>%
  summarise(
...
variability_overall_vapt <- variability_overall_vapt %>%
  left_join(TCV_all, by = "user_id") %>%
  left_join(CCV_all, by = "user_id") %>%
  mutate(cohort = "VAPT")

#write to csv for external analysis
write.csv(variability_overall_vapt,
          "vapt_overall_variability_raw.csv",
          row.names = FALSE)

#sliding window analysis for users with specified days of data
window_size <- 21
all_window_rows <- list()

#calculate BVS for all windows of all eligible users
...
#combine into one df
variability_windows_vapt <- bind_rows(all_window_rows)

#write to csv for external analysis
write.csv(variability_windows_vapt,
          "vapt_window_variability_raw.csv",
          row.names = FALSE)
